...
import laserdisc.fs2.parallel.SetUpLaserdiscCatsNettyBaseline.LaserdiscCatsNettyBaselineSetUp
import laserdisc.fs2.parallel.channels.ByteInByteOutNettyChannel
...
class LaserdiscCatsByteInByteOutNettyBaselineBench() {

  @Benchmark
  @OperationsPerInvocation(48)
  def parallelLoad1(setUp: LaserdiscCatsNettyBaselineSetUp, bh: Blackhole): Unit = {
  ...
  @Benchmark
  @OperationsPerInvocation(48)
  def parallelLoad2(setUp: LaserdiscCatsNettyBaselineSetUp, bh: Blackhole): Unit = {
...
object SetUpLaserdiscCatsNettyBaseline {

  @State(Scope.Benchmark)
  class LaserdiscCatsNettyBaselineSetUp {
    ...
    val resource = Blocker[IO] evalMap { bl =>
      RedisAddress("localhost", 6379).toInetSocketAddress[IO] map { address =>
        ByteInByteOutNettyChannel[IO](address, writeTimeout = Some(10.seconds), readMaxBytes = 8 * 1024 * 1024)(bl)
      }
    }
    ...
    def setup(): Unit =
      resource.allocated
        .map { case (rc, cu) =>
          testCases = TestCasesLaserdiscByteBaseline(rc)
          clientCleanUp = cu
        }
        .unsafeRunSync()
